...
const CardnodeMin = ({ imgsrc, alt, title, detail, role }) => {
    return (
        ...
        className={`relative card  p-6 bg-[#12122B]    md:p-3 lg:p-4 rounded-[13px] md:rounded-[17px] lg:rounded-[21px] delay-100 max-w-full duration-300 overflow-hidden  md:max-w-screen-sm transition-transform ease-in-out hover:-translate-y-1 hover:shadow-sm hover:shadow-sky-200 min-w-fit`}>
        {/* Name and logo */}
        <div 
            id='nameLogo'
            className="w-full grid grid-cols-2 gap-1 items-start"
            >
            <div className="grid grid-cols-2 sm:grid-cols-3 sm:gap-5  min-w-fit">
            <div
            className="h-12 w-12 min-w-fit rounded-full bg-none"    
            >
